...
* [How to Design a Distributed Messaging System like Kafka?](#how-to-design-a-distributed-messaging-system-like-kafka)
  ...
  * [Kafka Coordination & Metadata Flow – A Complete Explanation (Corrected)](#kafka-coordination--metadata-flow--a-complete-explanation-corrected)
  * [Mental Model (Anchor This First)](#mental-model-anchor-this-first)
  * [1. Broker Startup & Controller Election (`/controller`)](#1-broker-startup--controller-election-controller)
    ...
    * [Step 1.4: How brokers know who the controller is](#step-14-how-brokers-know-who-the-controller-is)
    * [Step 1.5: Watches (corrected and precise)](#step-15-watches-corrected-and-precise)
  * [2. How Metadata Is Built Inside Brokers](#2-how-metadata-is-built-inside-brokers)
    * [Step 2.1: Broker starts with empty metadata](#step-21-broker-starts-with-empty-metadata)
    * [Step 2.2: Initial metadata load (ONE TIME)](#step-22-initial-metadata-load-one-time)
    * [Step 2.3: Metadata updates (CORRECTED)](#step-23-metadata-updates-corrected)
    * [Do brokers store metadata for all topics?](#do-brokers-store-metadata-for-all-topics)
  * [3. Creating a New Topic (Clarified)](#3-creating-a-new-topic-clarified)
  ...
  * [4. Broker Goes Down](#4-broker-goes-down)
  ...
  * [5. How Producers Know Where to Send Data](#5-how-producers-know-where-to-send-data)
  ...
  * [6. Why Producers Never Talk to the Controller](#6-why-producers-never-talk-to-the-controller)
...
## Kafka Coordination & Metadata Flow – A Complete Explanation (Corrected)
...
## 1. Broker Startup & Controller Election (`/controller`)
...
### Step 1.4: How brokers know who the controller is

Any broker that needs to know the controller:

1. Reads `/controller` → gets `broker_id`
2. Reads `/brokers/ids/<broker_id>` → gets host + port

There is **no broker-to-broker discovery protocol**.

**ZooKeeper is the rendezvous point.**

---

### Step 1.5: Watches (corrected and precise)
...
## 2. How Metadata Is Built Inside Brokers

### Step 2.1: Broker starts with empty metadata
...
### Step 2.2: Initial metadata load (ONE TIME)

Each broker performs a **one-time read** from ZooKeeper:
...
From this, it builds an **in-memory metadata cache** containing:

* all brokers (id → host:port)
* all topics
* all partitions
...
### Step 2.3: Metadata updates (CORRECTED)

After startup:

* **Brokers do NOT watch `/brokers/topics`**
* **Only the controller watches ZooKeeper metadata**

When metadata changes:

1. ZooKeeper notifies the **controller**
2. Controller recomputes state if needed
3. Controller sends `UpdateMetadataRequest` RPCs to **all brokers**
4. Brokers update their **in-memory metadata cache**

This is **push-based propagation**, not ZooKeeper fan-out.

---

### Do brokers store metadata for all topics?

✅ **Yes. Every broker stores metadata for all topics (in memory).**

Why this is required:

1. Any broker can serve MetadataRequest
2. Any broker can become controller
3. Fast recovery without re-reading ZooKeeper

What brokers **do not** store:

* message data for all topics
* log segments for all partitions

---

## 3. Creating a New Topic (Clarified)
...
This version is now **architecturally correct, internally consistent, and interview-safe**.
